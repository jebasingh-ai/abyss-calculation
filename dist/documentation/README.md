...
**The Most Over-Engineered 1+1 Calculator in the Multiverse**

![Abyss Version](https://img.shields.io/badge/Abyss%20Version-2-brightgreen)
...
```bash
python3 -m abyss.example
```

C++

```bash
g++ src/cpp/abyss.cpp -o dist/abyss_cpp && ./dist/abyss_cpp
```

Fortran

```bash
gfortran src/fortran/abyss.f90 -o dist/abyss_fortran && ./dist/abyss_fortran
```
...
brainfuck src/brainfuck/abyss.bf
```
...
# Expected output:
# test_abyss.py::test_addition PASSED
```
...
· As an artistic statement about modern software development
· Because we stared too long into the compiler, and it stared back
· For the memes 🎭

🎯 Roadmap

· Add WebAssembly version
· Docker container that only outputs "2"
· Kubernetes deployment for high-availability 1+1 calculations
· Blockchain integration (AbyssCoin anyone?)
· Machine Learning model that predicts 1+1=2
· VR experience of the mathematical void
...
```
## made By
> The Ones Who Stared Too Long Into The Compiler
